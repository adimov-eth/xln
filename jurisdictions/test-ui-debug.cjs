...
async function main() {
  ...
  console.log(`[PIN] Using Depository: ${depositoryAddress}`);
  ...
  console.log(`\n[FIND] Testing Entity[RIGHTWARDS]Entity transfer:`);
  ...
  console.log(`\n[FIND] Batch structure:`, JSON.stringify({
    ...batch,
  ...
  try {
    ...
    console.log(`\n[TEST] Testing static call...`);
    const staticResult = await depository.processBatch.staticCall(entity1, batch);
    console.log(`[OK] Static call successful: ${staticResult}`);
    ...
    console.log(`[OK] Transaction successful: ${receipt.hash}`);
    ...
    if (balance1After === expectedBalance1 && balance2After === expectedBalance2) {
      console.log(`\n[DONE] CONTRACT WORKS PERFECTLY!`);
      console.log(`   The issue is in the UI ABI or parameters`);
    } else {
      console.log(`\n[X] Contract has issues`);
    }

  } catch (error) {
    console.error(`\n[X] Contract call failed:`, error.message);
    
    // Try to understand the error
    if (error.message.includes('require(false)')) {
      console.log(`\n[FIND] This is likely a validation error in the contract:`);
