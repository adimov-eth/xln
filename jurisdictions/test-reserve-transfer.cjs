...
async function main() {
  ...
  console.log("[OK] Depository deployed to:", await depository.getAddress());

  // Get signers
  const [owner, user1] = await hre.ethers.getSigners();
  console.log("[USER] Owner:", owner.address);
  console.log("[USER] User1:", user1.address);
  ...
  console.log("[OK] Transfer successful! Transaction:", receipt.hash);
  ...
  if (transferEvents.length > 0) {
    ...
    console.log(`[OUT] Transfer: ${event.args.from} [RIGHTWARDS] ${event.args.to} (${event.args.amount} of token ${event.args.tokenId})`);
  }
