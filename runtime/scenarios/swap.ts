...
let _processWithStep:
  | ((env: Env, inputs?: RoutedEntityInput[], delay?: number, single?: boolean) => Promise<Env>)
  | null = null;
...
export async function swap(env: Env): Promise<void> {
  ...
  try {
    ...
    console.log(
      `🔍 DEBUG: Alice has ${aliceRep.state.accounts.size} accounts, keys: ${Array.from(aliceRep.state.accounts.keys())
        .map(k => k.slice(-4))
        .join(', ')}`,
    );
    ...
    await process(env, [
      {
        ...
        entityTxs: [
          {
            type: 'extendCredit',
            data: { counterpartyEntityId: alice.id, tokenId: ETH_TOKEN_ID, amount: eth(ETH_CAPACITY_UNITS) },
          },
          {
            type: 'extendCredit',
            data: { counterpartyEntityId: alice.id, tokenId: USDC_TOKEN_ID, amount: usdc(USDC_CAPACITY_UNITS) },
          },
        ],
      },
      {
        ...
        entityTxs: [
          {
            type: 'extendCredit',
            data: { counterpartyEntityId: hub.id, tokenId: ETH_TOKEN_ID, amount: eth(ETH_CAPACITY_UNITS) },
          },
          {
            type: 'extendCredit',
            data: { counterpartyEntityId: hub.id, tokenId: USDC_TOKEN_ID, amount: usdc(USDC_CAPACITY_UNITS) },
          },
        ],
      },
    ]);
    ...
    assert(
      swapBookEntry1?.wantAmount === usdc(TRADE_USDC_MAIN_UNITS),
      `swapBook wantAmount = ${TRADE_USDC_MAIN_UNITS} USDC`,
    );
    ...
    assert(
      offer2?.giveAmount === expectedRemaining,
      `Remaining amount = ${expectedRemaining} (got ${offer2?.giveAmount})`,
    );
    ...
    assert(
      ethDelta3?.offdelta === -eth(TRADE_ETH),
      `Final ETH delta = -${TRADE_ETH} (Alice gave ${TRADE_ETH} ETH total)`,
    );
    const usdcDelta3 = aliceHubAccount3?.deltas.get(USDC_TOKEN_ID as TokenId);
    assert(
      usdcDelta3?.offdelta === usdc(TRADE_USDC_MAIN_UNITS),
      `Final USDC delta = +${TRADE_USDC_MAIN_UNITS} (Alice received ${TRADE_USDC_MAIN_UNITS} USDC)`,
    );
...
export async function swapWithOrderbook(env: Env): Promise<Env> {
  ...
  try {
    ...
    await process(env, [
      {
        ...
        entityTxs: [
          {
            type: 'extendCredit',
            data: { counterpartyEntityId: bob.id, tokenId: ETH_TOKEN_ID, amount: eth(ETH_CAPACITY_UNITS) },
          },
          {
            type: 'extendCredit',
            data: { counterpartyEntityId: bob.id, tokenId: USDC_TOKEN_ID, amount: usdc(USDC_CAPACITY_UNITS) },
          },
        ],
      },
      {
    ...
    console.log('═══════════════════════════════════════════════════════════════');
    console.log(
      `TEST: Alice SELL ${TRADE_ETH} ETH @ ${ETH_PRICE_MAIN}, Bob BUY ${TRADE_ETH_HALF} ETH @ ${ETH_PRICE_HIGH}`,
    );
    console.log('═══════════════════════════════════════════════════════════════\n');
    ...
    assert(
      aliceEthDelta === -aliceFilled.filledGive,
      `Alice ETH delta = -${aliceFilled.filledGive} (got ${aliceEthDelta})`,
    );
    assert(
      aliceUsdcDelta === aliceFilled.filledWant,
      `Alice USDC delta = +${aliceFilled.filledWant} (got ${aliceUsdcDelta})`,
    );
    ...
    if (aliceOfferRemaining) {
      ...
      assert(
        aliceOfferRemaining.giveAmount === expectedRemaining,
    ...
    const syncCounters = (leftId: string, rightId: string, label: string) => {
      ...
      const synced = Math.max(leftAccount.proofHeader.cooperativeNonce, rightAccount.proofHeader.cooperativeNonce);
...
export async function multiPartyTrading(env: Env): Promise<Env> {
  ...
  try {
      ...
      await process(env, [
        {
          ...
          entityTxs: [
            {
              type: 'extendCredit',
              data: { counterpartyEntityId: entity.id, tokenId: ETH_TOKEN_ID, amount: eth(ETH_CAPACITY_UNITS) },
            },
            {
              type: 'extendCredit',
              data: { counterpartyEntityId: entity.id, tokenId: USDC_TOKEN_ID, amount: usdc(USDC_CAPACITY_UNITS) },
            },
          ],
        },
        {
    ...
    const expectedCarolUsdc = usdcForEth(CAROL_SELL_ETH, ETH_PRICE_LOW) + usdcForEth(CAROL_SELL_2_ETH, ETH_PRICE_HIGH);
    ...
    const daveBuy1 = computeFilledAmounts(usdcForEth(DAVE_BUY_ETH, ETH_PRICE_MAIN), eth(DAVE_BUY_ETH), MAX_FILL_RATIO);
