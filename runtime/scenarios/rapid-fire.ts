...
export async function rapidFire(env: Env): Promise<void> {
  ...
  try {
    ...
    for (let batch = 0; batch < paymentCount / batchSize; batch++) {
      ...
      if (batch % 2 === 0) {
        console.log(
          `   Batch ${batch + 1}/10: ${batchSize * 2} payments in ${elapsed}ms (${forwardCount} fwd, ${reverseCount} rev)`,
        );
      }
    }
    ...
    assert(ahDelta >= -tolerance && ahDelta <= tolerance, `Alice-Hub delta within tolerance: ${ahDelta}`);
    assert(hbDelta >= -tolerance && hbDelta <= tolerance, `Hub-Bob delta within tolerance: ${hbDelta}`);
