...
export async function insuranceCascadeScenario(
  ...
  try {
    ...
    setFrameNarrative(
      env,
      '5. Burning Reserves',
      'H1 burns 5,000 USDC by sending it to a burn address, making it shrink significantly.',
    );
    if (browserVM) {
    ...
    if (browserVM) {
      ...
      const insuranceRegs = [
        { insured: H1, insurer: H2, tokenId: USDC_TOKEN_ID, limit: usd(3000), expiresAt: oneYear },
      ];
      const insuranceSig = await browserVM.signSettlement(leftEntity, rightEntity, [], [], insuranceRegs);
      await browserVM.settleWithInsurance(leftEntity, rightEntity, [], [], insuranceRegs, insuranceSig);
    }
    ...
    setFrameNarrative(
      env,
      '9. Default',
      "H1's reserves are now 10,000 USDC, but off-chain losses cause a 2,000 USDC shortfall in a settlement with Alice.",
    );
    if (browserVM) {
    ...
    setFrameNarrative(
      env,
      '10. Insurance Claim',
      "The 2,000 USDC shortfall triggers H1's insurance. H2 pays Alice directly.",
    );
    if (browserVM) {
    ...
    setFrameNarrative(
      env,
      '11. Final State',
      `Alice is made whole. H2's reserves are down. H1 has a new debt to its insurer, H2.`,
    );
    ...
    setFrameNarrative(
      env,
      '12. Recovery',
      'Later, H1 receives new funds. The `enforceDebts` mechanism automatically repays its debt to H2 first.',
    );
    if (browserVM) {
