...
export async function swapMarket(env: Env): Promise<void> {
  ...
  try {
    ...
    assert(
      aliceEthOfferV2.giveAmount === eth(10),
      `Alice ETH ask v2 giveAmount = ${eth(10)} (got ${aliceEthOfferV2.giveAmount})`,
    );
    assert(
      aliceEthOfferV2.wantAmount === usdc(30200),
      `Alice ETH ask v2 wantAmount = ${usdc(30200)} (got ${aliceEthOfferV2.wantAmount})`,
    );
    ...
    for (const trader of [carol, dave, frank]) {
      ...
      const account =
        rep.state.accounts.get(hubEth.id as AccountKey) ||
        rep.state.accounts.get(hubWbtc.id as AccountKey) ||
        rep.state.accounts.get(hubDai.id as AccountKey);
      if (account) {
