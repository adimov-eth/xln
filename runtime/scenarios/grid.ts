...
export async function grid(env: Env): Promise<void> {
  ...
  try {
    ...
    console.log(
      `📐 Creating ${GRID_DIMS.x}×${GRID_DIMS.y}×${GRID_DIMS.z} 3D grid (${GRID_DIMS.x * GRID_DIMS.y * GRID_DIMS.z} nodes)...`,
    );
    ...
    await pushSnapshot(
      env,
      'GRID-CREATED',
      `${gridEntities.length} nodes in ${GRID_DIMS.x}×${GRID_DIMS.y}×${GRID_DIMS.z} 3D grid`,
      {
