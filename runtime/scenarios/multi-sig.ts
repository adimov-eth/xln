  ...
  try {
    ...
    for (const validator of alice.validators) {
      ...
      if (hasAccount) {
        ...
        console.log(
          `    → Account height: ${account.height}, mempool: ${account.mempool.length}, pending: ${!!account.proposal}`,
        );
      }
    }
      ...
      runtimeTxs: [
        {
          ...
          signerId: '5',
          data: { isProposer: true, position: { x: 200, y: 0, z: 0 }, config: testConfig },
        },
      ],
      entityInputs: [],
    });
    ...
    assert(
      t1Rep.proposal!.collectedSigs?.size === 1,
      `Only proposer signature, got ${t1Rep.proposal!.collectedSigs?.size || 0}`,
    );
    ...
    assert(
      t1AfterWait.state.height === heightBefore,
      `Height should NOT change with only 1/3 signatures: ${t1AfterWait.state.height} === ${heightBefore}`,
    );
      ...
      [
        {
          ...
          signerId: '1',
          entityTxs: [
            { type: 'extendCredit', data: { counterpartyEntityId: hub.id, tokenId: USDC, amount: usd(10_000) } },
          ],
        },
      ],
      offlineSigners,
    ...
    assert(
      ...
      `Frame should commit with 2/3 (1+2): ${s1AfterOffline.state.height} > ${heightBeforeOffline}`,
    );
    assert(!s1AfterOffline.proposal, 'Proposal should be cleared after commit');
    console.log(
      `   ✅ Frame committed with 3 offline (height ${heightBeforeOffline} → ${s1AfterOffline.state.height})`,
    );
    ...
    assert(
      s1AfterPropose.proposal!.collectedSigs?.size === 1,
      `Proposal should have 1 sig (self), got ${s1AfterPropose.proposal!.collectedSigs?.size || 0}`,
    );
