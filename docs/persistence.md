...
async function storeFrame(frame: Frame) {
  ...
  await cas.put(`${CAS_PREFIX}idx:${frame.entityId}:${frame.height}`, hash)
}
```
...
```typescript
async function replayFromWAL(state: ServerState, fromHeight: bigint): Promise<ServerState> {
  for await (const [key, value] of db.iterator({
...
```typescript
function verifyRecovery(recovered: ServerState, expectedHash: string): boolean {
