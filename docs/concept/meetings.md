...
— _Okay, Egor, that's why the question comes up—what kinds do we have? Essentially we have jurisdictions, entities, and channels._

_Channels live inside entities. Jurisdictions live outside—they're high-performance Rust processes, so there's no point virtualising them within entities. So we only care about:_
...
2. _Channel-level transactions (inside a channel). After the entity decides something, it pushes a transaction into the channel machine—e.g., "shift delta."_
...
_Flow:_ a transaction arrives at the HUB proposer (one of four nodes). It runs locally, goes to the other three validators for signatures, then drops into the channel machine for the user that originated it. Inside the channel we check collateral, signatures, etc., bubble an event back up to the entity level, match orders, trigger side-effects, and finally propagate deltas to other channels. So you must track when state changes happen in a channel versus in an entity.*

_Each transaction type has its own parameters: at entity level—`createProposal`, `voteProposal`, etc.; at channel level—`exchange`, `transfer`, whatever._
...
**Simple example: a transaction between two channels—how is it executed?**  
— _Let's minimise the setup. One signer, no multi-sig for now._  
_User A wants to pay User B through HUB H:_

1. _A's entity auto-activates a proposal that adds a "createPayment" TX to the channel with H._
...
4. _Entity H puts a matching TX into B's channel (minus fee). If B's channel is live it ships immediately; otherwise it sits in mempool._
...
— _Channel state (balances, nonces, signatures) lives inside the channel machine's DB path. At the entity level we store shared things: order-books, payment-book, proposals._

_The DB uses LevelDB with prefixes: `/server/<id>`, `/entity/<id>`, `/channel/<entityId>/<counterpartyId>`._
...
**Merkle trees?**  
— _Only needed if an entity holds 100+ channels—mainly hubs. Personal users with <10 channels can hash everything in one go. For hubs you use a shallow Merkle tree (maybe three levels) to avoid re-hashing a million leaves._
...
- _Channel consensus: 2-of-2 signatures (duplex channel)._
