...
test('recovery after crash during block commit', async () => {
  ...
  expect(recoveredState.ok).toBe(true);
  if (recoveredState.ok) {
    // Should have first block committed
    expect(recoveredState.value.height).toBe(height(1));
...
test('recovery with multiple restarts', async () => {
  ...
  const result1 = await runner1.processBlock(server);
  expect(result1.ok).toBe(true);
  ...
  if (recovered1.ok) {
    ...
    const result2 = await runner2.processBlock(server);
  ...
  const finalState = await runner3.recover();
  
  expect(finalState.ok).toBe(true);
  if (finalState.ok) {
    expect(finalState.value.height).toBe(height(2));
    
    const aliceEntity = finalState.value.signers.get(signer(0))?.get(id('alice'));
    const bobEntity = finalState.value.signers.get(signer(1))?.get(id('bob'));
    
    // NOTE: This test verifies snapshot recovery works correctly.
    // The balances reflect that cross-entity messages are not processed
    // in this test setup, which is a limitation of the test, not the recovery.
    expect(aliceEntity).toBeDefined();
    expect(bobEntity).toBeDefined();
    expect(aliceEntity?.data.balance).toBe(950n);
    expect(bobEntity?.data.balance).toBe(500n);
  }
