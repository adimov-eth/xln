...
export const addr = (pb: PubKey): Hex => {
  const h = keccak(pb);
  return bytesToHex(h.slice(-20));   // take rightmost 20 bytes of keccak(pubkey) as address
};
...
export const verifyAggregate = (
  ...
  const aggPub = bls.aggregatePublicKeys(pubs);
  return bls.verify(hexToBytes(hanko), hexToBytes(msgHash), aggPub);
};
