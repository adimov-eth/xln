...
const genesisEntity = (): Replica => {
  const quorum: Quorum = {
    threshold: 3,  // require 3 out of 5 signatures to commit (e.g., 5 members with weight1, threshold3)
  ...
  return {
    ...
    proposer: ADDRS[0] as Hex,        // initial proposer (could be rotated later)
...
export class Runtime {
  private state: { replicas: Map<string, Replica>; height: bigint } = { replicas: new Map(), height: 0n };
  ...
  async tick(now: number, incoming: Input[]) {
    ...
    // Step 2: fulfill signature placeholders in outbox (this is where private keys are used)
    const fulfilledOutbox = await Promise.all(outbox.map(async msg => {
      if (msg.cmd.type === 'SIGN' && msg.cmd.sig === '0x00') {
        ...
        const { signer, frameHash } = msg.cmd;
        const signerIndex = ADDRS.findIndex(a => a === signer);
        const priv = PRIVS[signerIndex];
        if (priv) {
          msg.cmd.sig = await sign(Buffer.from(frameHash.slice(2), 'hex'), priv);
        }
      }
      if (msg.cmd.type === 'COMMIT' && msg.cmd.hanko === '0x00') {
        ...
        const sigsMap = (msg.cmd.frame as any).sigs as Map<string, Hex>;
        msg.cmd.hanko = aggregate([...sigsMap.values()] as Hex[]);
