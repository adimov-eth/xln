...
import { createMessage } from '../engine/router.js';
...
export const processCommand = (context: CommandContext): Result<CommandResult> => {
  ...
  if (command.type === 'addTx') {
    return addTransactionToMempool(entity, command.tx, signer, meta);
  }
...
const addTransactionToMempool = (
  ...
  const isBroadcast = (transaction as any)._broadcast === true;
  const txToStore = isBroadcast ? { ...transaction, _broadcast: undefined } : transaction;

  const updatedEntity = {
    ...entity,
    mempool: [...entity.mempool, txToStore]
  };

  const messages: OutboxMsg[] = !isBroadcast && meta.quorum.length > 1
    ? meta.quorum
        .filter(s => s !== signer)
        .map(toSigner => ({
          from: meta.id,
          to: meta.id,
          toSigner,
          command: { type: 'addTx' as const, tx: { ...transaction, _broadcast: true } }
        }))
    : [];
