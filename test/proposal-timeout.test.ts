...
describe('proposal timeout', () => {
  it('allows re-proposal after timeout', async () => {
    ...
    const { next: state1 } = await applyServerFrame(state, [attach1, attach2, attach3], () => 0n, 0n)
    
    // Add a transaction
    const tx: Input = [0, entityId, { 
      type: 'addTx', 
      tx: { kind: 'test', data: {}, nonce: 1n, sig: '0x1111111111111111111111111111111111111111signed' }
    }]
    const { next: state2 } = await applyServerFrame(state1, [tx], () => 1n, 1n)
    
    // First proposer proposes
    const propose1: Input = [0, entityId, {
      type: 'proposeFrame',
    ...
    const propose2: Input = [1, entityId, {
      type: 'proposeFrame', 
